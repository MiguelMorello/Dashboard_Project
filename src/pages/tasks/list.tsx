...
const List = ({children}: React.PropsWithChildren) => {
  ...
  const handleAddCard = (args:{stageId: string}) => {
    const path = args.stageId === 'unassigned' ? '/task/new' : `/task/new?stageId=${args.stageId}`
    replace(path);
  }
  ...
  return (
    ...
    <KanbanBoardContainer>
      ...
      <KanbanBoard onDragEnd={handleOnDragEnd}>
        ...
        <KanbanColumn id='unassigned' title={'unassigned'} count={taskStages.unassignedStage.length || 0} 
        ...
        {taskStages.columns?.map((column) => (
          ...
          <KanbanColumn key={column.id} id={column.id} title={column.title} count={column.tasks.length} 
